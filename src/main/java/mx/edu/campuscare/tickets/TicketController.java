...
public class TicketController {
  ...
  public List<Ticket> all() {
    return repo.findAll();
  } // TRAINING GAP U2-P1: filtra por ownership/rol.

  @GetMapping("/{id}")
  public Ticket one(@PathVariable Long id) {
    return repo.findById(id).orElseThrow();
  } // TRAINING GAP U3-P1: BOLA.

  @PostMapping
  @ResponseStatus(HttpStatus.CREATED)
  public Ticket create(@Valid @RequestBody CreateTicket req, Authentication a) {
    return repo.save(new Ticket(a.getName(), req.title(), req.description(), false));
  }

  @PatchMapping("/{id}")
  public Ticket patch(@PathVariable Long id, @RequestBody Map<String, Object> body) { // TRAINING GAP U3-P2: binding
                                                                                      // permisivo.
